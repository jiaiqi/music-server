...
module.exports = {
  ...
  async uploadImg() {
    ...
    // 上传基础目录
    const uploadBasePath = "app/public/upload/img/"
    // 生成文件名
    const filename = Date.now() + "" + Number.parseInt(Math.random() * 1) + path.extname(stream.filename)
    // 生成文件夹
    const dirName = dayjs(Date.now()).format("YYYYMMDD")
    if (!fs.existsSync(path.join(this.config.baseDir, uploadBasePath, dirName))) fs.mkdirSync(path.join(this.config.baseDir, uploadBasePath, dirName))
    // 生成写入路径
    ...
    try {
      ...
      imgUrl = "http://" + this.ctx.request.header.host + "/public/upload/img/" + dirName + "/" + filename
  ...
  async uploadSong() {
    ...
    // 上传基础目录
    const uploadBasePath = "app/public/upload/song/"
    // 生成文件名
    ...
    try {
      // 写入文件
      await awaitStreamReady(stream.pipe(writeStream))
      songUrl = "http://" + this.ctx.request.header.host + "/public/upload/song/" + dirName + "/" + filename
  ...
  async uploadLyric() {
    ...
    // 上传基础目录
    const uploadBasePath = "app/public/upload/lyric/"
    // 生成文件名
    const filename = Date.now() + "" + Number.parseInt(Math.random() * 10000) + path.extname(stream.filename)
    // 生成文件夹
    const dirName = dayjs(Date.now()).format("YYYYMMDD")
    ...
    try {
      // 写入文件
      await awaitStreamReady(stream.pipe(writeStream))
      lyricUrl = "http://" + this.ctx.request.header.host + "/public/upload/lyric/" + dirName + "/" + filename
